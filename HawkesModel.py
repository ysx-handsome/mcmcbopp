...
class HawkesModel:
    ...
    def loglike_one_a(self,seq_one,cluster_idx):
            ...
            for i in range(Nc):
                ...
                if i > 0:
                    ...
                    lambdai = lambdai + np.sum(pij)     
                Loglike = Loglike - np.log(lambdai)
            # relabel_logger.info(f'Loglike:{Loglike}')
    ...
    def loglike_one_a_fordebug(self,seq_one,mean,prec):
            # relabel_logger.info(f'means:{self.b_a[:,cluster_idx]}')
            # relabel_logger.info(f'prec:{self.beta[:,cluster_idx]}')
            Time = seq_one['Time']
            Event = seq_one['Mark']
            Event_int = Event.astype(int) if isinstance(Event, np.ndarray) else int(Event)
            Tstart = seq_one['Start']
...
if __name__=='__main__':
    ...
    para1['A'] = 0.0000000000000005 * para1['A'] / max_eigval
    para1['w'] = 0
