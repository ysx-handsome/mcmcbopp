...
def softmax(x):
    """计算数组 x 的 softmax。"""
    e_x = np.exp(x-np.max(x))  # 减去最大值来提高数值稳定性或者使用tempered？
...
class ConditionalMCMC_model: 
    def __init__(self, pp_mix, h, g, params):
        ...
        self.iter = 0
        # self.logger_means = logging.getLogger('logger_means')
        # self.logger_means.setLevel(logging.INFO)

        # means_logger
        # means_file_name = 'dpp_N:'+str(self.pp_mix.N)+' '+'dpp_nu:'+str(self.pp_mix.nu)+' '+'dpp_rho'+str(self.pp_mix.rho)
        # means_file_name = str(datetime.now())+' '+means_file_name
        # means_file_handler = logging.FileHandler(means_file_name+'--means.log')
        # means_file_handler.setLevel(logging.INFO)
        # formatter2 = logging.Formatter('%(asctime)s - %(name)s - %(levelname)s - %(message)s')
        # means_file_handler.setFormatter(formatter2)
        # self.logger_means.addHandler(means_file_handler)

        #prec_logger
        # self.logger_prec = logging.getLogger('logger_prec')
        # self.logger_prec.setLevel(logging.INFO)
        # prec_file_name = 'dpp_N:'+str(self.pp_mix.N)+' '+'dpp_nu:'+str(self.pp_mix.nu)+' '+'dpp_rho'+str(self.pp_mix.rho)
        # prec_file_name = str(datetime.now())+' '+prec_file_name
        # prec_file_handler = logging.FileHandler(prec_file_name+'--prec.log')
        # prec_file_handler.setLevel(logging.INFO)
        # formatter2 = logging.Formatter('%(asctime)s - %(name)s - %(levelname)s - %(message)s')
        # prec_file_handler.setFormatter(formatter2)
        # self.logger_prec.addHandler(prec_file_handler)

        # 全log
        self.full_logger = logging.getLogger('full_logger')
        self.full_logger.setLevel(logging.INFO)
        full_file_name = 'dpp_N:'+str(self.pp_mix.N)+' '+'dpp_nu:'+str(self.pp_mix.nu)+' '+'dpp_rho'+str(self.pp_mix.rho)
        full_file_name = str(datetime.now())+' '+full_file_name
        full_file_handler = logging.FileHandler(full_file_name+'full.log')
        ...
        # #relabel
        # self.relabel_logger = logging.getLogger('relabel_logger')
        # self.relabel_logger.setLevel(logging.INFO)
        # relabel_file_name = 'dpp_N:'+str(self.pp_mix.N)+' '+'dpp_nu:'+str(self.pp_mix.nu)+' '+'dpp_rho'+str(self.pp_mix.rho)
        # relabel_file_name = str(datetime.now())+' '+relabel_file_name
        # relabel_file_handler = logging.FileHandler(relabel_file_name+'relabel.log')
        # relabel_file_handler.setLevel(logging.INFO)
        # relabel_formatter = logging.Formatter('%(asctime)s - %(name)s - %(levelname)s - %(message)s')
        # relabel_file_handler.setFormatter(relabel_formatter)
        # self.relabel_logger.addHandler(relabel_file_handler)
    def initialize(self, data, hakes_model):
        ...
        hakes_model.b_a =  self.a_means.T #   self.a_means shape = k * event_type 每一行表示 一个envent_type 的 k个采样值  hakes_model.b_a eventtype*k
    ...
    def run_one(self):
        ...
        # self.full_logger.info(f'a_means:{self.a_means}')
        self.pp_mix.update_hypers(self.a_means, self.na_means)
        
        self.full_logger.info(f'iteration: {self.iter}')
        self.full_logger.info(f'a_means:{self.a_means}')
        self.full_logger.info(f'a_precs:{self.a_precs}')
        self.full_logger.info(f'na_means:{self.na_means}')
        self.full_logger.info(f'na_prec:{self.na_precs}')
        self.full_logger.info(f'label:{self.clus_alloc}')
    ...
    def sample_allocations_and_relabel(self):
        ...
        for iter,i in enumerate(range(self.ndata)):
            ...
            probas[Ma:] = curr_na_jumps
            probas = np.log(probas)
            # self.relabel_logger.info(f'data_index:{iter}')
            for k in range(Ma):
                # probas[k] += self.lpdf_given_clus(datum, curr_a_means[k, :], curr_a_precs[k])
                # probas[k] += self.hakes_model.loglike_one_a(datum,k,self.relabel_logger)
                probas[k] += self.hakes_model.loglike_one_a(datum,k)
                # self.relabel_logger.info(f'k:{k}')
                # self.relabel_logger.info(f'log_like:{self.hakes_model.loglike_one_a(datum,k,self.relabel_logger)}')
                # replace with transformer
            for k in range(Mna):
                # probas[k + Ma] += self.lpdf_given_clus(datum, curr_na_means[k, :], curr_na_precs[k])
                # print(k)
            ...
            # self.relabel_logger.info(f'loglike:{probas}')
            probas_soft = softmax(probas)
            '''#####################
            self.logger2.info(f'data i: {self.iter%self.ndata}')
            self.logger2.info(f'probability: {probas}')
            '''
            #newalloc = np.random.choice(np.arange(Mtot), p=probas) - 1
            # print('probas',probas)
            # import pdb
            ...
            # self.relabel_logger.info(f'probas:{probas_soft}')
            newalloc = np.random.choice(np.arange(Mtot), p=probas_soft)
            self.clus_alloc[i] = newalloc

        # print(self.a_means.shape)
        # self.full_logger.info(f'before:{self.a_means}')
        self._relabel()
        # self.full_logger.info(f'after:{self.a_means}')
        # print(self.a_means.shape)
    ...
    def _relabel(self):
        ...
        for k in range(Ma):
            if (self.clus_alloc == k).sum() == 0:
                a2na.add(k)
        # self.full_logger.info(f'na2a:{na2a}')
        # self.full_logger.info(f'a2na:{a2na}')
        a2na_vec = list(a2na)
        n_new_na = len(a2na)
        new_na_means = self.a_means[a2na_vec, :]
        new_na_precs = [self.a_precs[i] for i in a2na_vec]
    ...
    def sample_means(self): # sample base intensity 
        ...
        for i in range(self.a_means.shape[0]):  #对当前这个cluster,即对cluster遍历
            ...
            # self.logger_means.info(f'iter:{self.iter}')
            # self.logger_means.info(f'i:{i}')
            # self.logger_means.info(f'sigma:{sigma}')
            # self.logger_means.info(f'currmean:{currmean}')
            # self.logger_means.info(f'prop:{prop}')
            # self.logger_means.info(f'currlike:{currlik}')
            # self.logger_means.info(f'proplike:{proplik}')
            # self.logger_means.info(f'like_ration:{lik_ratio}')
            # self.logger_means.info(f'dense_other:{self.pp_mix.dens(others)}')
            # self.logger_means.info(f'other:{others}')
            # self.logger_means.info(f'dense_all:{self.pp_mix.dens(np.vstack([prop, others]))}')
            # self.logger_means.info(f'all:{np.vstack([prop, others])}')
            # self.logger_means.info(f'prop_papangelous:{self.pp_mix.papangelou(prop,others)}')
            # self.logger_means.info(f'currmean_papangelous:{self.pp_mix.papangelou(currmean,others)}')
            # self.logger_means.info(f'prior_ration:{prior_ratio}')
            # self.logger_means.info(f'arate:{arate}')
    ...
    def sample_vars(self): # sample A
        ...
        for i in range(self.a_means.shape[0]):
            ...
            accepted = False
            # self.logger_prec.info(f'iter:{self.iter}')
            # self.logger_prec.info(f'i:{i}')
            # self.logger_prec.info(f'sigma:{sigma}')
            # self.logger_prec.info(f'curr_precs:{curr_precs}')
            # self.logger_prec.info(f'prop:{prop}')
            # self.logger_prec.info(f'currlike:{currlik}')
            # self.logger_prec.info(f'proplike:{proplik}')
            # self.logger_prec.info(f'like_ration:{lik_ratio}')
            # self.logger_prec.info(f'prop_lpdf:{self.g.lpdf(prop)}')
            # self.logger_prec.info(f'curr_prec_lpdf:{self.g.lpdf(curr_precs)}')
            # self.logger_prec.info(f'prior_ration:{prior_ration}')
            # self.logger_prec.info(f'arate:{arate}')
            if np.log(np.random.rand()) < arate:
                #print('A update')
                accepted = True
                self.a_precs[i] = prop
