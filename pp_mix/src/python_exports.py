...
def run_pp_mix_hks(burnin, niter, thin, data, params, hakes_model,args,log_every):
    ...
    # logging.basicConfig(filename='history4.log', level=logging.INFO, 
...
#####保存为字典########
    ...
    for i in range(niter):
        ...
        if (i + 1) % log_every == 0:
            ...
            # history['a_precs'].append([cura_prec_i.data for cura_prec_i in curr.a_precs])
            # history['na_precs'].append(curr.na_precs)
            # history['a_jumps'].append([cura_jump_i.data for cura_jump_i in curr.a_jumps])
            # history['na_jumps'].append(curr.na_jumps)
            history['clus_alloc'].append(curr.clus_alloc)
            # history['u'].append(curr.u)
            # history['ppstate'].append(curr.pp_state)
            # log_history(this_log,curr, i)
            #history['gamma'].append(curr.pp_state.gamma)
            #history['beta'].append(curr.pp_state.beta)
            #history['r'].append(curr.pp_state.r)
            # print(curr.clus_alloc)
        if (i+1)%500 == 0:
            file_name = 'dpp_N'+str(pp_mix.N)+' '+'dpp_nu'+str(pp_mix.nu)+' '+'dpp_rho'+str(pp_mix.rho)+'iter'+str(i)+'delta_mu'+str(args.mudelta)+'initnclus'+str(args.init_n_clus)+'K'+str(args.K)
            file_name = str(datetime.now())+' '+file_name
            print("start making files")
            np.save('/media/disk1/chatgpt/aaapointprocess/mcmcbmm/20240104npy/'+file_name+'.npy', history)
    print("finish")
    return out
...
def _run_pp_mix(burnin, niter, thin, data, params, hakes_model, args,bernoulli=False, hawkes = True,log_every = 10):
