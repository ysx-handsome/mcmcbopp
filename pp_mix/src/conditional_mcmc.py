...
class hawkesmcmc(ConditionalMCMC_model):
    ...
    def initialize_allocated_means(self, ranges):
        ...
        # # event_type * k 每一行代表一种mark的k个采样值
        # print('means_init_done',samples.shape)
        # samples = np.array([[0.4,2],[0.4,2],[0.4,2]])
        # sigma = 1
        # print('ranges',ranges)
        # center = [(low*2+high/2)/2 for low,high in ranges]
        # samples = np.array([np.random.normal(loc=c,scale=sigma,size=init_n_clus) for c in center])
        # event_type * K
            

        # samples = np.array([np.random.uniform(low, high/2, init_n_clus) for low, high in ranges])
        #epsilon = np.random.uniform(-1/4,1/4,1)[0]
        # samples = np.array([0.5 ,1,1.1],[0.5,1,1.1],[0.5,1,1.1])
        #print(epsilon)
        print("ranges is ",ranges)
        # samples = np.array([np.linspace(low*2, high/2, init_n_clus) for low, high in ranges])
        if init_n_clus == 2:
            samples = np.array([np.linspace(low, high/2, init_n_clus) for low, high in ranges])
        else:
            samples = []
            for low, high in ranges:
                points = np.linspace(low, high/2, init_n_clus + 1)
                # 取相邻点的中值
                averaged_points = (points[:-1] + points[1:]) / 2
                samples.append(averaged_points)
            # 将结果转换为 NumPy 数组
            samples = np.array(samples)
        # v1:samples = np.array([[0.5 + epsilon,1.1 + epsilon],[0.5 + epsilon,1.1 + epsilon],[0.5 + epsilon,1.1 + epsilon]])
